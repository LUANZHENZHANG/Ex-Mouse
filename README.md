...
**[下载顺鼠 Ex-Mouse 1.11 DMG 安装包](https://github.com/LUANZHENZHANG/Ex-Mouse/releases/download/v1.11/Ex-Mouse-1.11-macOS-arm64.dmg)**

- 文件名：`Ex-Mouse-1.11-macOS-arm64.dmg`
- 大小：约 2 MB
...
授权是顺鼠正常工作的关键。顺鼠需要以下三项 macOS 权限：

| 权限 | 用途 | 未授权时的表现 |
| --- | --- | --- |
| 辅助功能 | 监听和修改滚轮、中键及侧键事件 | 滚动和鼠标手势可能无效 |
| 输入监控 | 在其他应用中接收全局鼠标事件 | 菜单可能显示监听创建失败 |
| 自动化 | 通过 System Events 发送桌面切换快捷键 | 鼠标切换桌面无效 |

首次启动顺鼠后：

1. 点击 macOS 菜单栏中的鼠标图标。
2. 打开 **调试** 子菜单。
3. 点击 **打开辅助功能设置**，在系统设置中允许“顺鼠”。
4. 点击 **打开输入监控设置**，在系统设置中允许“顺鼠”。
5. 点击 **打开自动化设置**，允许“顺鼠”控制 `System Events`。
6. 如果列表中没有“顺鼠”，点击 `+`，从“应用程序”中手动添加“顺鼠.app”。
7. 三项权限处理完成后，从菜单栏退出顺鼠，再重新打开一次。

也可以手动进入：

`系统设置 → 隐私与安全性 → 辅助功能 / 输入监控 / 自动化`

“自动化”项目有时要在第一次使用侧键或中键手势切换桌面后才会出现。如果没有看到，
先触发一次桌面切换，再回到系统设置检查。
...
4. 完全退出顺鼠后重新启动；只关闭菜单或系统设置通常不会让新权限立即生效。
...
3. 将“顺鼠.app”移到废纸篓。
4. 打开 `系统设置 → 隐私与安全性`。
5. 如需彻底清理，在“辅助功能”“输入监控”和“自动化”中删除顺鼠记录。
