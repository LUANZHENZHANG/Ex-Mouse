...
<p align="center"><strong>500K级轻量、无遥测，让鼠标和触控板一样丝滑！</strong></p>

我在 Mac 使用中发现一些小问题：Magic Mouse 不好用、普通鼠标在 Mac 上无法使用触控板类似的手势、滚轮方向也和触控板相反。

高强度干活用触控板还是太累，我又不愿意使用太复杂的 Mac 鼠标设置软件，所以让 Codex 设计了这个小工具。

顺鼠 / Ex-Mouse 轻量、无遥测，主要功能：
- 让 Mac 触控板与鼠标滚轮各自保持顺手的滚动方向。
- 前侧键切换到下一个桌面，后侧键切换到上一个桌面。
- 按住中键左右滑动切换桌面；向上或向下滑动放大或缩小页面。
- 双击中键打开调度中心。

注：各项功能可单独关闭，以防快捷键、手势与其他软件冲突。

这个项目特别鸣谢Codex！活都是他干的;-D。反正我自己用着挺好，所以分享给有同样需要的朋友。
如果使用中有任何问题，或有什么想要加进去的功能，请联系我。谢谢！
...
目前公开稳定版为 [顺鼠 Ex-Mouse 1.28](https://github.com/LUANZHENZHANG/Ex-Mouse/releases/download/v1.28/Ex-Mouse-1.28-macOS-arm64.dmg)。
...
3. 右键打开新版顺鼠。
4. 如果功能未就绪，点击 **开启或修复辅助功能权限…**，然后在系统设置中重新允许顺鼠。
5. 首次使用桌面切换或页面缩放时，按系统提示允许“顺鼠”控制 `System Events`；这是发送系统快捷键所需的单独授权。
...
- **启用中键+手势功能**：按住中键左右滑动切换桌面，向上滑动放大页面、向下滑动缩小页面。
- **启用中键侧键快捷键**：前侧键切换到下一个桌面，后侧键切换到上一个桌面；双击中键打开调度中心。
